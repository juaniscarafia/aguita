...
const WaterTracker = () => {
  ...
  const { requestPermission, startNotifications, stopNotifications, wasEnabled } =
    useNotifications();

  // 1) Al montar: si el permiso ya está otorgado y el usuario lo dejó activo, reanudar
  useEffect(() => {
    const autoResume = async () => {
      if (Notification?.permission === "granted" && wasEnabled?.()) {
        // reanuda el interval con la config guardada (intervalMs)
        startNotifications();
        setNotificationsEnabled(true);
      } else {
        setNotificationsEnabled(false);
    ...
    autoResume();
  }, [startNotifications, wasEnabled]);

  // 2) Cambio de día -> reset + toast
  useEffect(() => {
    const today = getTodayKey();
    if (fecha !== today) {
      reiniciarDia();
  ...
  const toggleNotifications = async () => {
    if (notificationsEnabled) {
      ...
      const granted = await requestPermission();
      if (granted) {
        startNotifications(); // usa intervalMs de noti-config
        toast.success("🔔 Notificaciones activadas");
        setNotificationsEnabled(true);
      } else {
        toast.error("❌ Permiso denegado");
